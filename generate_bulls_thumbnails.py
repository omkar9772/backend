"""
Generate thumbnails for existing bull images in the main bulls table
"""
...
def generate_thumbnails_for_bulls():
    """Generate thumbnails for all bulls that have photos but no thumbnails"""
    ...
    print(f"📸 Found {len(bulls)} bulls that need thumbnails\n")
    ...
    for i, bull in enumerate(bulls, 1):
        try:
            ...
            if not original_blob.exists():
                print(f"  ⚠️  Original image not found: {bull.photo_url}")
                error_count += 1
                continue
